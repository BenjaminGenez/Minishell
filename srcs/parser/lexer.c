...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/09/12 10:39:50 by user              #+#    #+#             */
/*   Updated: 2025/09/29 12:00:00 by user             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void handle_quotes_state(char **input, int *i, char *quote, int *in_quotes)
{
    if (((*input)[*i] == '\'' || (*input)[*i] == '"') && !(*in_quotes))
    {
        *quote = (*input)[*i];
...
static t_token *handle_word(char **input, int *i)
{
    ...
    while ((*input)[*i])
    {
        handle_quotes_state(input, i, &quote, &in_quotes);
        if (!in_quotes && (ft_isspace((*input)[*i]) ||
...
static t_token *process_input(char *input, int *i)
{
    t_token *new_token;
...
t_token *tokenize(char *input)
{
    ...
    while (input[i])
    {
        new_token = process_input(input, &i);
        if (!new_token)
            continue;
        if (!new_token)
        {
